...
@tool
def calculate_cost_saving() -> str:
    """Graviton 전환 시 예상 비용 절감을 계산합니다."""
    # ap-northeast-2 기준 온디맨드 가격
    costs = {
        "t3.medium": {"hourly": 0.052, "arch": "x86_64"},
        "t4g.medium": {"hourly": 0.0416, "arch": "ARM64 (Graviton)"},
    }

    current = costs["t3.medium"]["hourly"]
    graviton = costs["t4g.medium"]["hourly"]
    saving_pct = (current - graviton) / current * 100
    monthly_saving = (current - graviton) * 24 * 30 * 2  # 온디맨드 2대 기준

    return json.dumps({
        "current_instance": "t3.medium",
        "target_instance": "t4g.medium",
        "current_hourly": f"${current}",
        "graviton_hourly": f"${graviton}",
        "saving_percentage": f"{saving_pct:.1f}%",
        "monthly_saving_2nodes": f"${monthly_saving:.2f}",
        "note": "Spot 노드 추가 전환 시 더 큰 절감 가능"
    }, ensure_ascii=False)

@tool
def graviton_migration_checklist() -> str:
    """Graviton 전환 체크리스트를 반환합니다."""
    checklist = {
        "1_dockerfile": "Dockerfile에 --platform=linux/arm64 추가",
        "2_ecr_build": "GitHub Actions에서 ARM64 이미지 빌드 추가",
        "3_karpenter": "NodePool requirements에 arm64 추가",
        "4_node_type": "온디맨드 노드를 t4g.medium으로 변경",
        "5_test": "ARM64 빌드 후 기능 테스트",
        "status": "미완료 - 구현 필요"
    }
...
비용 최적화, Graviton 전환, 리소스 사용률 분석을 담당합니다.
구체적인 수치와 실행 가능한 액션 아이템을 제시합니다.""",
    tools=[get_prometheus_metrics, calculate_cost_saving, graviton_migration_checklist],
)
...
def run_multi_agent_analysis():
    ...
    devops_result = devops_guru(
        ...
        "Graviton 전환 시 절감액을 계산하고, "
        "전환을 위한 체크리스트와 우선순위를 제시해줘."
    )
...
3. **중기 (1개월)**: DevOpsGuru의 Graviton 전환 계획 실행 → 비용 30% 절감
"""
