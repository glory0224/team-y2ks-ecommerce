...
GRAFANA_URL      = os.environ.get("GRAFANA_URL", "http://prometheus-grafana.monitoring.svc.cluster.local:80")
...
def _prom_query(expr: str) -> str:
    """Prometheus instant query를 실행하고 첫 번째 값을 반환합니다."""
    try:
        url = f"{PROMETHEUS_URL}/api/v1/query"
        req = urllib.request.Request(f"{url}?{urllib.parse.urlencode({'query': expr})}")
...
def _grafana_request(path: str) -> dict:
    """Grafana HTTP API 요청을 실행합니다."""
    import base64
...
def _extract_panels(panels: list) -> list:
    """패널 목록에서 중첩 row 패널을 평탄화합니다."""
    result = []
    for panel in panels:
        if panel.get("type") == "row":
            result.extend(_extract_panels(panel.get("panels", [])))
        else:
            result.append(panel)
    return result
...
def _substitute_vars(expr: str) -> str:
    """Grafana 템플릿 변수를 기본값으로 치환합니다."""
    import re as _re
    result = expr
...
def _prom_range_all(expr: str, legend: str = "") -> list:
    """Prometheus range query (1h, 60s step). [{label, data:[[ts,val],...]}] 반환."""
    try:
        end   = int(time.time())
        start = end - 3600
        url   = f"{PROMETHEUS_URL}/api/v1/query_range"
        params = urllib.parse.urlencode({"query": expr, "start": start, "end": end, "step": "60"})
        ...
        for r in results[:4]:
            ...
            if legend and "{" not in legend:
                ...
                lbl = next(iter(metric.values()), str(metric))[:40]
            else:
                lbl = expr[:40]
            values = [
                [int(v[0]), float(v[1])]
            ...
            if values:
                series.append({"label": lbl, "data": values})
        return series
    except Exception:
        return []
...
def list_grafana_dashboards() -> str:
    """Grafana에 등록된 모든 대시보드 목록(제목, UID, 태그)을 조회합니다."""
    try:
        dashboards = _grafana_request("/api/search?type=dash-db&limit=100")
        now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        result = {
            "__type": "dashboard_list",
            "generated_at": now,
            "total": len(dashboards),
            "dashboards": [
                {"title": d["title"], "uid": d.get("uid", ""), "tags": d.get("tags", [])}
                for d in dashboards
            ],
        }
        return json.dumps(result, ensure_ascii=False)
    except Exception as e:
        return f"대시보드 목록 조회 실패: {e}"


@tool
def get_grafana_dashboard_report(dashboard_uid: str) -> str:
    """특정 Grafana 대시보드(uid 지정)의 패널별 시계열 데이터를 차트용 JSON으로 반환합니다. list_grafana_dashboards로 uid를 먼저 확인하세요."""
    try:
        data   = _grafana_request(f"/api/dashboards/uid/{dashboard_uid}")
        dash   = data.get("dashboard", {})
        title  = dash.get("title", dashboard_uid)
        panels = _extract_panels(dash.get("panels", []))

        now    = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        report = {"__type": "chart_report", "title": title, "generated_at": now, "panels": []}

        for panel in panels[:40]:
            panel_title = panel.get("title", "").strip()
            raw_targets = [t for t in panel.get("targets", []) if t.get("expr")]
            if not raw_targets or not panel_title:
                continue
            # 변수 치환 후 사용 가능한 표현식만 수집
            targets = []
            for t in raw_targets:
                expr_raw = t.get("expr", "")
                expr_sub = _substitute_vars(expr_raw)
                # 치환 후에도 알 수 없는 $ 변수가 남으면 스킵
                import re as _re2
                if _re2.search(r'\$[a-zA-Z_]', expr_sub):
                    continue
                targets.append({**t, "expr": expr_sub})
            if not targets:
                continue
            unit = panel.get("fieldConfig", {}).get("defaults", {}).get("unit", "")
            desc = panel.get("description", "")
            panel_data: dict = {"title": panel_title, "unit": unit, "description": desc, "series": []}
            for t in targets[:3]:
                expr   = t.get("expr", "")
                legend = t.get("legendFormat", "") or ""
                for s in _prom_range_all(expr, legend):
                    panel_data["series"].append(s)
                    if len(panel_data["series"]) >= 5:
                        break
            if panel_data["series"]:
                report["panels"].append(panel_data)
                if len(report["panels"]) >= 20:
                    break

        return json.dumps(report, ensure_ascii=False)
    except Exception as e:
        return f"대시보드 리포트 조회 실패 (uid={dashboard_uid}): {e}"
...
OBSERVE_TOOLS = [get_node_resources, get_pod_resources,
                 calculate_cost_savings, list_grafana_dashboards,
                 get_grafana_dashboard_report, send_slack_message]
...
## 절대 규칙 (위반 금지)
- 반드시 툴을 먼저 호출하여 실제 데이터를 수집한 후 분석할 것
- 툴 호출 없이 수치(CPU%, 파드 수, 노드 수 등)를 추정하거나 생성하는 것 엄격히 금지
- 조회되지 않은 데이터는 "확인 불가"로 명시할 것
- PR 번호, 브랜치명 등 확인되지 않은 정보 생성 금지
...
## 절대 규칙 (위반 금지)
- 반드시 툴을 먼저 호출하여 실제 데이터를 수집한 후 분석할 것
- 툴 호출 없이 수치(봇 비율, 참여자 수, 당첨자 수 등)를 추정하거나 생성하는 것 엄격히 금지
- 조회되지 않은 데이터는 "확인 불가"로 명시할 것
...
## 절대 규칙 (위반 금지)
- 반드시 툴을 먼저 호출하여 실제 데이터를 수집한 후 분석할 것
- 툴 호출 없이 수치(CPU%, 메모리%, 비용 등)를 추정하거나 생성하는 것 엄격히 금지
- 조회되지 않은 데이터는 "확인 불가"로 명시할 것
...
- 그라파나 대시보드 목록 요청 → list_grafana_dashboards (전체 대시보드 uid·태그 목록)
- 특정 대시보드 리포트 → list_grafana_dashboards로 uid 확인 후 get_grafana_dashboard_report(uid) 호출
- 2~3개만 선택
...
### Grafana 목록/리포트 요청인 경우 (최우선 규칙)
- list_grafana_dashboards 또는 get_grafana_dashboard_report 툴 결과를 **절대 요약하지 말고 그대로 전체 출력**
- 추가 분석 코멘트는 툴 출력 아래에 간략히 추가 가능
- 형식: 툴이 반환한 마크다운 표/목록 전체 출력
...
### observe (리소스/비용/모니터링 전문가)
담당: 노드별 CPU 사용률, 메모리 사용률, 파드별 리소스 사용량,
     OnDemand vs Spot 비용 비교, 리소스 병목, 성능 모니터링, 비용 최적화,
     그라파나 대시보드 리포트, k6 부하테스트 메트릭, KEDA ScaledObject 메트릭,
     Karpenter 노드 프로비저닝 메트릭, Prometheus 쿼리 결과
...
- "노드 리소스 현황" → ["observe"]
- "그라파나 대시보드 목록" → ["observe"]
- "그라파나 리포트 보여줘" → ["observe"]
- "Kubernetes 대시보드 리포트" → ["observe"]
- "모니터링 리포트" → ["observe"]
- "k6 메트릭 알려줘" → ["observe"]
- "KEDA 메트릭 현황" → ["observe"]
- "Karpenter 메트릭 보여줘" → ["observe"]
- "Node Exporter 현황" → ["observe"]
- "CoreDNS 대시보드 조회" → ["observe"]
...
def _direct_grafana(user_message: str) -> dict | None:
    ...
    try:
        ...
        report_kws = ["리포트", "report", "보고서", "전체", "모든", "모두", "all"]
        if any(kw in msg.lower() if kw.isascii() else kw in msg for kw in report_kws):
            result = _all_dashboards_report()
            return {"observe": result, "final": result}

        # 기본: 대시보드 목록 반환 (보여줘/확인/조회/목록 등 일반 요청)
        result = list_grafana_dashboards()
        return {"observe": result, "final": result}
    except Exception as e:
        err = json.dumps({"__type": "error", "message": f"Grafana 조회 실패: {e}"}, ensure_ascii=False)
        return {"observe": err, "final": err}


def _all_dashboards_report() -> str:
    """모든 대시보드의 패널 데이터를 조회하여 통합 리포트 반환."""
    dashboards_raw = _grafana_request("/api/search?type=dash-db&limit=100")
    if not isinstance(dashboards_raw, list) or not dashboards_raw:
        return list_grafana_dashboards()

    panels_all = []
    dashboard_titles = []
    for d in dashboards_raw:  # 전체 대시보드 스캔
        uid = d.get("uid", "")
        title = d.get("title", uid)
        if not uid:
            continue
        try:
            report_json = get_grafana_dashboard_report(uid)
            report = json.loads(report_json)
            new_panels = report.get("panels", [])
            if not new_panels:
                continue
            dashboard_titles.append(title)
            for panel in new_panels:
                panel["dashboard"] = title
                panels_all.append(panel)
            if len(panels_all) >= 40:  # 전체 최대 40개 패널
                break
        except Exception:
            continue
...
async def run_agent(user_message: str) -> dict:
    ...
    # Grafana 요청은 LLM이 툴 결과를 요약하므로 직접 처리
    direct = _direct_grafana(user_message)
